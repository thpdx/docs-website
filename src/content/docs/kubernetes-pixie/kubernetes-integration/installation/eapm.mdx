...
## Key differences between eAPM and APM [#key-differences]
...
7. *(Optional)* To view pod deployment status in real-time, run:
    ...
    ```
  <Callout variant="tip" title="tip">
  The eAPM Helm chart offers multiple configuration options. Review the [`value.yaml`](https://github.com/newrelic/helm-charts/blob/master/charts/nr-ebpf-agent/values.yaml) file to tailor the installation to your needs.
  </Callout>




## Access the eAPM-powered APM UI [#eapm-access]
...
## How eAPM agent and APM works together [#eapm-apm]
